...
function AddInfoSection() {
  ...
  const handleJobRemove = (job: any) => {
    dispatch(removeJob(job));
  };

  const handleJobInput = (e: any, job: any) => {
    console.log(e.target);
    dispatch(
      changeJobInput({
        values: { name: e.target.name, value: e.target.value },
        id: job.id,
      })
    );
  };
    ...
    <Container>
      ...
      <ScrollArea style={{ height: '90vh' }} scrollHideDelay={0}>
        <form onSubmit={formSubmit}>
          ...
          {/* job role  */}
          <div>
            {/* map over available job roles */}
            {state.jobs.map((job, i) => (
              <Grid key={job.id} align='center' gutter={'md'}>
                <Grid.Col span={6}>
                  <TextInput
                    mb='xs'
                    id={id}
                    ...
                    onChange={(e) => handleJobInput(e, job)}
                    withAsterisk
                  />
                </Grid.Col>
                <Grid.Col span={i === 0 ? 6 : 4}>
                  <TextInput
                    mb='xs'
                    id={id}
                    variant='filled'
          ...
          <Button mb='md' onClick={handleJobAdd}>
            Add a Job Role
          </Button>
